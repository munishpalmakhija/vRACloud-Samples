...
Import-CSV ./vrac-environment-vmc.csv | ForEach-Object { 
	...
	New-vRA-CloudAccount-VMC -VMC_API_KEY $vmctoken -VMC_SDDC_Name $vmcsddcname -vCenterHostname $vCenterIP -Credential $vccredential -VMC_NSX_IP $vmcnsxip -CloudProxyName $cloudproxyname -CloudAccountName $cloudaccountname | Out-Null
